...
@MainActor
enum ATTManager {
    /// Feature switch for the ATT prompt. Enabled for the Tenjin MMP
    /// integration: Tenjin benefits from IDFA access when the user grants
    /// tracking. The prompt is triggered at the end of onboarding (see
    /// `OnboardingView.finishOnboarding`), and the MMP `connect()` runs
    /// inside the prompt's completion so IDFA is available for the first
    /// install event when granted.
    ...
    /// RevenueCat requires — without it RC will not deliver events to Tenjin.
